import { NavLink } from "react-router-dom";

const Header = (props) => {
    return (
        <header className="header">
            <a className="logo"><img src="Imagenes/Logo irridelta2.jpeg" alt="Logo Irridelta" /></a>

            <nav className="menu">
                <div className="holder-menu">
                    <ul>
                        <li className="activo"><NavLink to="/" className={({isActive}) => isActive ? "activo" : undefined}>Inicio</NavLink></li>
                        <li><NavLink to="nosotros" className={({isActive}) => isActive ? "activo" : undefined}>Nosotros</NavLink></li>
                        <li><NavLink to="productos" className={({isActive}) => isActive ? "activo" : undefined}>Productos</NavLink></li>
                        <li><NavLink to="contacto" className={({isActive}) => isActive ? "activo" : undefined}>Contacto</NavLink></li>
                        <li href="https://www.instagram.com/irridelta/?hl=es" target="_blank"><img class="ig-icono" alt="icono ig"
                            src="data:image/png;base64," /></li>
                        <li href="#"><img class="fbk-icono" alt="icono fb"
                            src="data:image/png;base64," /></li>
                    </ul>
                </div>
            </nav>
        </header>
    );
}
export default Header;